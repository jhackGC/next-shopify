...
async function quickTest() {
  ...
  if (!token || token.includes('your_')) {
    console.log('❌ SHOPIFY_STOREFRONT_PRIVATE_ACCESS_TOKEN is not configured');
    return;
  }
  ...
  try {
    ...
    console.log(`📦 Products: ${products.edges.length > 0 ? `Found (e.g., "${products.edges[0].node.title}")` : 'None found'}`);
    console.log(`📚 Collections: ${collections.edges.length > 0 ? `Found (e.g., "${collections.edges[0].node.title}")` : 'None found'}`);

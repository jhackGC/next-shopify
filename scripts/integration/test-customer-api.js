...
async function validateOAuthConfiguration() {
  ...
  if (clientId && !clientId.includes('your_')) {
    ...
    if (appUrl) {
      ...
      log.info('Make sure this redirect URI is registered in your Partner Dashboard');
    }
...
async function testAPIEndpointAccessibility() {
  ...
  try {
    ...
    if (response.status === 401) {
      log.success('✅ API endpoint is accessible (401 Unauthorized is expected without token)');
      return true;
    } else if (response.status === 200) {
      log.success('✅ API endpoint is accessible and responding');
      return true;
    } else if (response.status === 404) {
      log.error('❌ API endpoint not found - check your SHOPIFY_CUSTOMER_ACCOUNT_API_URL');
      return false;
    } else {
...
async function testNextJSAPIRoutes() {
  ...
  for (const route of authRoutes) {
    try {
      ...
      if (error.message.includes('ECONNREFUSED')) {
        log.warn(`${route} - Server not running (this is normal if dev server is not started)`);
      } else {
  ...
  if (allRoutesAccessible) {
    log.info('💡 To fully test API routes, start your dev server with: npm run dev');
  }
...
async function runCustomerAPITests() {
  ...
  for (const test of tests) {
    try {
      ...
      const passed = typeof result === 'boolean' ? result : result.configValid || result.valid;
  ...
  if (passed === total) {
    ...
    log.info('You can now test the full authentication flow in your application.');
  } else {
